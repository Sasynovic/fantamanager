...
require_once __DIR__ . '/../../models/trattative.php';
...
try {
    ...
    // Esegui l'update
    $result = $trattative->update($id, $data);

    if ($result) {
        http_response_code(200);
        echo json_encode([
            "success" => true,
            "message" => "Trattativa aggiornata con successo.",
            "status" => "success"
        ]);
    } else {
        ...
        echo json_encode([
            "success" => false,
            "message" => "Nessuna trattativa trovata con questo ID o nessun cambiamento applicato.",
            "status" => "error"
        ]);
    }
