...
require_once __DIR__ . '/../../models/presidenti.php';
...
try {
    ...
    $id = $_GET['id'] ?? die(json_encode([
        "message" => "ID presidente mancante",
        "status" => "error"
    ]));
    ...
    // Esegui l'update
    $result = $presidenti->update($id, $data);
    ...
    if ($result) {
        ...
        echo json_encode([
            "success" => false,
            "message" => "Nessuna presidente trovata con questo ID o nessun cambiamento applicato.",
            "status" => "error"
        ]);
    }
